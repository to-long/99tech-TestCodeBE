...
import { permissions, rolePermissions, roles, userRoles, users } from '../../db/schema/iam';
...
export async function createUser(
  body: { email: string; password: string; name: string; status?: string; roleCodes?: string[] },
  actor: Actor,
  ...
  await writeAudit({
    ...
    action: 'create',
    after: { email: body.email, name: body.name, roles: roleRows.map((r) => r.code).sort() },
